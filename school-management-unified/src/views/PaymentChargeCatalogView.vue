<template>
  <DashboardLayout>
    <div class="fk-page" :dir="isRTL ? 'rtl' : 'ltr'">
      <FikrPageHeader
        :title="$t('systemSettings.feeItemsLines')"
        :subtitle="$t('systemSettings.feeItemsPageIntro')"
      />
      ...
      <div class="fk-card">
        <header class="flex flex-wrap items-center justify-between gap-3 border-b border-fikr-hairline px-5 py-4 sm:px-6">
          <div class="min-w-0">
            <h2 class="fk-card__title truncate">{{ $t('paymentSettings.chargeItemsListHeading') }}</h2>
          </div>
          <div class="flex shrink-0 flex-nowrap items-center gap-2">
              ...
              <button
                ...
                :aria-label="$t('paymentSettings.addChargeItem')"
                @click="openCreate"
              >
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div class="p-6">
          ...
          <template v-else-if="rows.length">
            ...
            >
              {{ $t('paymentSettings.noChargeFilterResults') }}
            </p>
          ...
          <div v-else class="flex min-h-[16rem] flex-col items-center justify-center text-center">
            ...
            <p class="text-sm font-medium text-gray-600">{{ $t('paymentSettings.emptyCharges') }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      >
        ...
        <div class="fk-drawer__body">
          <div class="fk-form__row">
            <label class="fk-flabel" for="charges-search"><span>{{ $t('common.search') }}</span></label>
            <input
              id="charges-search"
              v-model="searchQuery"
              type="search"
              class="fk-field"
              :placeholder="$t('paymentSettings.searchChargesPlaceholder')"
            >
          </div>
          <div class="fk-form__row">
            <label class="fk-flabel" for="charges-status"><span>{{ $t('common.status') }}</span></label>
            <select
              id="charges-status"
    ...
    <FikrDialog
      :show="showForm"
      plain-footer
      :title="editingRow ? $t('paymentSettings.editChargeItem') : $t('paymentSettings.addChargeItem')"
      @close="closeForm"
    >
      <form id="charge-item-form" class="fk-form" @submit.prevent="saveForm">
        <div class="fk-form__section">
          <div class="fk-form__row">
            <label class="fk-flabel" for="charge-code"><span>{{ $t('paymentSettings.code') }}</span></label>
            <input
              id="charge-code"
              v-model="form.code"
              required
          ...
          <div class="fk-form__row">
            <label class="fk-flabel" for="charge-label"><span>{{ $t('paymentSettings.label') }}</span></label>
            <input
              id="charge-label"
              v-model="form.label"
              required
      ...
      <template #footer>
        ...
        <button type="submit" form="charge-item-form" class="fk-btn fk-btn--primary" :disabled="saving">
...
async function saveForm() {
  ...
  try {
    ...
    if (editingRow.value) {
      const updated = await paymentConfigService.updateChargeType(editingRow.value.id, payload)
