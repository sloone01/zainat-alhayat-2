    ...
    <div class="space-y-6 pb-10" :dir="isRTL ? 'rtl' : 'ltr'">
      <section class="rounded-2xl border border-gray-200/80 bg-white p-6 shadow-sm">
        <router-link
          ...
          class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-primary-200/80 bg-primary-100 text-primary-700 shadow-sm hover:bg-primary-200 mb-3"
        ...
        <h1 class="text-xl font-bold text-gray-900">{{ levelTitle }}</h1>
        <p class="text-sm text-gray-600 mt-1">{{ $t('feesV2.gradeLinkSubtitle') }}</p>
      </section>
      ...
      <form class="space-y-6" @submit.prevent="save">
        ...
        <div v-if="form.fee_package_id && form.lines.length" class="rounded-2xl border border-gray-200/80 bg-white overflow-hidden shadow-sm">
          <div class="border-b border-gray-100 bg-primary-50/50 px-6 py-3">
            ...
            <p class="text-xs text-primary-700/80 mt-0.5">{{ $t('feesV2.zeroAllowed') }}</p>
          </div>
...
async function load() {
  ...
  const lv = summaries.find((s) => s.id === levelId.value)
  levelTitle.value = lv ? (locale.value === 'ar' ? lv.name_ar : lv.name_en) || lv.name : levelId.value

  packages.value = await feesV2Service.listPackages(schoolId.value)
  const link = await feesV2Service.getGradeLink(schoolId.value, levelId.value).catch(() => null)
  if (link) {
...
async function save() {
  saving.value = true
  try {
    await feesV2Service.saveGradeLink({
