<template>
  <DashboardLayout>
    <div class="fk-page" :dir="isRTL ? 'rtl' : 'ltr'">
      <FikrPageHeader
        :title="$t('systemSettings.discountItemsLines')"
        :subtitle="$t('systemSettings.discountItemsPageIntro')"
      />
      ...
      <div class="fk-card">
        <header class="flex flex-wrap items-center justify-between gap-3 border-b border-fikr-hairline px-5 py-4 sm:px-6">
          ...
          <div class="flex shrink-0 flex-nowrap items-center gap-2">
              ...
              <button
                ...
                :aria-label="$t('paymentSettings.addDiscountItem')"
                @click="openCreate"
              >
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div class="p-6">
          ...
          <template v-else-if="rows.length">
            ...
            >
              {{ $t('paymentSettings.noDiscountFilterResults') }}
            </p>
      ...
      >
        ...
        <div class="fk-drawer__body">
          <div class="fk-form__row">
            <label class="fk-flabel" for="discounts-search"><span>{{ $t('common.search') }}</span></label>
            <input
              id="discounts-search"
              v-model="searchQuery"
              type="search"
              class="fk-field"
              :placeholder="$t('paymentSettings.searchDiscountsPlaceholder')"
            >
          </div>
          <div class="fk-form__row">
            <label class="fk-flabel" for="discounts-status"><span>{{ $t('common.status') }}</span></label>
            <select
              id="discounts-status"
    ...
    <FikrDialog
      :show="showForm"
      plain-footer
      :title="editingRow ? $t('paymentSettings.editDiscountItem') : $t('paymentSettings.addDiscountItem')"
      @close="closeForm"
    >
      <form id="discount-item-form" class="fk-form" @submit.prevent="saveForm">
        <div class="fk-form__section">
          <div class="fk-form__row">
            <label class="fk-flabel" for="discount-code"><span>{{ $t('paymentSettings.code') }}</span></label>
            <input
              id="discount-code"
              v-model="form.code"
              required
          ...
          <div class="fk-form__row">
            <label class="fk-flabel" for="discount-label"><span>{{ $t('paymentSettings.label') }}</span></label>
            <input
              id="discount-label"
              v-model="form.label"
              required
      ...
      <template #footer>
        ...
        <button type="submit" form="discount-item-form" class="fk-btn fk-btn--primary" :disabled="saving">
