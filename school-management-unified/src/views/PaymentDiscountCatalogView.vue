  ...
  <DashboardLayout>
    <div class="space-y-4" :dir="isRTL ? 'rtl' : 'ltr'">
      <div class="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
        ...
        <h1 class="text-xl font-bold text-gray-900 mt-2">{{ $t('systemSettings.discountItemsLines') }}</h1>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex items-center mb-4">
          <div class="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center me-3 shrink-0">
        ...
        <form class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end" @submit.prevent="addRow">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-900 mb-1">{{ $t('paymentSettings.valueOptional') }}</label>
            <input v-model="form.value" class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-primary-500 focus:border-primary-500" />
          </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div v-if="!rows.length" class="px-4 py-10 text-center text-gray-500">
          {{ $t('paymentSettings.emptyDiscounts') }}
        </div>
            ...
            <table class="min-w-full text-sm">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="text-start px-4 py-3 font-semibold text-gray-700">{{ $t('paymentSettings.valueOptional') }}</th>
              ...
              <tbody>
                <tr v-for="row in paginatedItems" :key="row.id" class="border-t border-gray-200 hover:bg-gray-50">
                  ...
                  <td class="px-4 py-3 text-gray-600">{{ row.value || '—' }}</td>
                  <td class="px-4 py-3">
                    <span
            ...
            >
              ...
              <dl class="px-4 py-3 text-sm">
                <div class="rounded-lg bg-gray-50 px-3 py-2">
                  <dt class="text-xs font-medium text-gray-500">{{ $t('paymentSettings.valueOptional') }}</dt>
                  <dd class="mt-0.5 font-medium text-gray-900">{{ row.value || '—' }}</dd>
                </div>
              </dl>
...
const form = ref({ code: '', label: '', value: '' })
...
async function addRow() {
  ...
  flashOk.value = ''
  try {
    const row = await paymentConfigService.createDiscountType(schoolId.value, {
...
async function toggle(row: PaymentCatalogRow, is_active: boolean) {
  try {
    const updated = await paymentConfigService.updateDiscountType(row.id, { is_active })
