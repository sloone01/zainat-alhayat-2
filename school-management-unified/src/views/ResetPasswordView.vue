...
onMounted(() => {
  ...
  token.value = typeof raw === 'string' ? raw.trim() : ''
  if (!token.value) {
    feedback.error(t('resetPassword.missingToken'), t('common.error'))
  }
})
...
async function handleSubmit() {
  ...
  if (!newPassword.value || !confirmPassword.value) {
    feedback.error(t('resetPassword.fillRequired'), t('common.error'))
    return
  }
  ...
  try {
    ...
    feedback.success(t('resetPassword.success'), t('common.success'))
    ...
    } else if (/expired|invalid/i.test(msg) || err?.code === 'VALIDATION_ERROR') {
      feedback.error(t('resetPassword.invalid'), t('common.error'))
    } else {
      feedback.error(t('resetPassword.failed'), t('common.error'))
    }
  } finally {
    loading.value = false
  }
