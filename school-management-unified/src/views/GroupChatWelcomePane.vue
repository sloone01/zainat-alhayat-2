<template>
  <div class="flex min-h-0 flex-1 flex-col" :dir="isRTL ? 'rtl' : 'ltr'">
    <ChatThreadShell>
      <ScrollArea6>
        <div class="flex min-h-[18rem] flex-col items-center justify-center py-16 text-center">
          <p class="text-sm font-semibold text-fikr-ink">{{ $t('chatRooms.mailboxEmptyTitle') }}</p>
          <p class="mt-1 text-xs text-fikr-ink-soft">{{ $t('chatRooms.mailboxEmptyHint') }}</p>
        </div>
        <template #composer>
          <ChatComposer
            input-id="gc-welcome-composer"
            disabled
