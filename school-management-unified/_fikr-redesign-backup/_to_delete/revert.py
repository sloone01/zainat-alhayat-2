...
root=sys.argv[1]
def rw(path, fn):
    p=os.path.join(root,path); s=open(p).read(); open(p,'w').write(fn(s)); print('ok',path)

rw('src/components/FikrPageHeader.vue', lambda s: '''<template>
  <section class="fk-hero">
    <div class="fk-hero__pixels" aria-hidden="true">
      <span v-for="n in 16" :key="n" />
    </div>
    <div class="relative flex flex-wrap items-end justify-between gap-4">
      <div class="min-w-0 max-w-2xl">
        <p v-if="eyebrow" class="fk-hero__eyebrow">{{ eyebrow }}</p>
        <h1 class="fk-hero__title">{{ title }}</h1>
        <p v-if="subtitle" class="fk-hero__subtitle">{{ subtitle }}</p>
      </div>
...
def inv(pairs):
    def f(s):
        for a,b in pairs:
            if b not in s: print('  MISSING:', b[:50])
            s=s.replace(b,a)
        return s
    return f
...
rw('src/assets/main.css', inv(css_pairs))

rw('src/views/GradeLevelsView.vue', inv([
  ('''                <td>
  ...
  ('<td><span class="fk-chip fk-chip--outline font-mono" dir="ltr">{{ grade.code }}</span></td>','<td><span class="font-mono text-xs text-fikr-ink-muted" dir="ltr">{{ grade.code }}</span></td>'),
  ('''            <h2 class="fk-card__title">{{ $t('systemSettings.gradesManagement') }}</h2>
            <p class="fk-card__meta">{{ grades.length }}</p>''','''            <h2 class="fk-card__title">{{ $t('systemSettings.gradesManagement') }}</h2>
            '''),
]))
rw('src/views/SystemSettingsView.vue', inv([('class="fk-grid--2 items-start"','class="grid grid-cols-1 gap-5 xl:grid-cols-2 items-start"')]))
rw('src/views/SchoolLandingEditorView.vue', inv([
  ('''          <span
            class="fk-chip"
  ...
  ('<div class="flex flex-wrap gap-2 border-b border-fikr-hairline pb-4">','<div class="flex flex-wrap gap-6 border-b border-fikr-hairline">'),
]))
rw('src/views/GroupManagementView.vue', inv([
  ('''                class="fk-item"
              >
                <div
                  ...
                  aria-hidden="true"
                />
                <div class="flex flex-1 flex-col p-5">''','''                class="fk-item"
              >
                <div class="flex flex-1 flex-col p-5">'''),
  ('class="fk-monogram fk-monogram--navy h-11 w-11 rounded-xl"','class="fk-monogram fk-monogram--navy"'),
  ('<div class="mt-auto flex items-center justify-end rounded-b-card border-t border-fikr-hairline bg-fikr-pearl px-4 py-2.5">','<div class="mt-auto flex items-center justify-end rounded-b-card border-t border-fikr-hairline px-4 py-2">'),
]))
rw('src/views/UserManagementView.vue', inv([('<div class="px-4 py-4">','<div class="px-4 pt-4 pb-3">')]))
rw('src/layouts/DashboardLayout.vue', inv([
...
rw('src/views/LoginView.vue', inv([
...
'''),
  ...
  ('class="fk-input rounded-xl px-4 py-3.5 text-base"','class="fk-input px-3.5 py-3 text-[15px]"'),
  ('class="fk-input rounded-xl px-4 py-3.5 pe-12 text-base tracking-[0.15em]"','class="fk-input px-3.5 py-3 pe-11 text-[15px]"'),
  ('<legend class="mb-2 text-sm font-medium text-fikr-ink">','<legend class="mb-2 text-[13px] font-medium text-fikr-ink-muted">'),
  ('class="rounded-xl border px-3 py-3 text-sm font-medium transition-colors','class="rounded-[10px] border px-3 py-2.5 text-sm font-medium transition-colors'),
  ...
  ('class="fk-btn fk-btn--primary mt-3 w-full rounded-xl py-3.5 text-base"','class="fk-btn fk-btn--primary mt-2 w-full py-3 text-[15px]"'),
  ('<form class="mt-7 space-y-3" @submit.prevent="handleLogin">','<form class="mt-6 space-y-3" @submit.prevent="handleLogin">'),
  ('class="fk-btn fk-btn--pearl fk-btn--tool px-4 py-2 text-sm font-medium"','class="fk-btn fk-btn--pearl fk-btn--sm"'),
  ('class="text-sm font-medium text-primary-600 hover:text-primary-700 hover:underline"','class="text-[13px] font-medium text-primary-600 hover:underline"'),
]))
