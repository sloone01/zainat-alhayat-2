  ...
  <DashboardLayout>
    <div class="fk-page" :dir="isRTL ? 'rtl' : 'ltr'">
      ...
      <div class="fk-card max-w-4xl p-5 sm:p-6">
        <div class="flex flex-wrap gap-2 border-b border-fikr-hairline pb-4">
          <button
            v-for="tab in tabs"
            :key="tab"
            type="button"
            class="rounded-pill px-3.5 py-1.5 text-sm font-medium transition-colors"
            :class="activeTab === tab ? 'bg-navy-800 text-white' : 'text-fikr-ink-muted hover:bg-fikr-parchment hover:text-fikr-ink'"
            @click="activeTab = tab"
          >
        ...
        <div v-else class="mt-4 space-y-4">
          <template v-if="activeTab === 'Branding'">
            <label class="block text-sm">
              <span class="fk-label">{{ $t('schoolLandingEditor.logoUrl') }}</span>
              <input v-model="form.logo_url" class="input mt-1" />
            </label>
            <label class="block text-sm">
              <span class="fk-label">{{ $t('schoolLandingEditor.heroImageUrl') }}</span>
              <input v-model="form.hero_image_url" class="input mt-1" />
            </label>
            <div class="grid sm:grid-cols-2 gap-3">
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.brandNameEn') }}</span>
                <input v-model="form.brand_name_en" class="input mt-1" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.brandNameAr') }}</span>
                <input v-model="form.brand_name_ar" class="input mt-1" dir="rtl" />
              </label>
            </div>
            <label class="block text-sm">
              <span class="fk-label">Slug (/s/…)</span>
          ...
          <template v-else-if="activeTab === 'Hero'">
            <div class="grid sm:grid-cols-2 gap-3">
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.badgeEn') }}</span>
                <input v-model="form.badge_en" class="input mt-1" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.badgeAr') }}</span>
                <input v-model="form.badge_ar" class="input mt-1" dir="rtl" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.titleEn') }}</span>
                <input v-model="form.hero_title_en" class="input mt-1" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.titleAr') }}</span>
                <input v-model="form.hero_title_ar" class="input mt-1" dir="rtl" />
              </label>
            </div>
            <label class="block text-sm">
              <span class="fk-label">{{ $t('schoolLandingEditor.subtitleEn') }}</span>
              <textarea v-model="form.hero_subtitle_en" rows="3" class="input mt-1" />
            </label>
            <label class="block text-sm">
              <span class="fk-label">{{ $t('schoolLandingEditor.subtitleAr') }}</span>
              <textarea v-model="form.hero_subtitle_ar" rows="3" class="input mt-1" dir="rtl" />
            </label>
            <div class="grid sm:grid-cols-2 gap-3">
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.ctaPrimaryEn') }}</span>
                <input v-model="form.cta_primary_en" class="input mt-1" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.ctaPrimaryAr') }}</span>
                <input v-model="form.cta_primary_ar" class="input mt-1" dir="rtl" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.ctaSecondaryEn') }}</span>
                <input v-model="form.cta_secondary_en" class="input mt-1" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.ctaSecondaryAr') }}</span>
          ...
          <template v-else-if="activeTab === 'Features'">
            <div
              ...
              :key="idx"
              class="fk-card--pearl p-4 space-y-2"
            >
              <div class="flex justify-between items-center">
                <span class="text-xs font-semibold text-fikr-ink-soft">#{{ idx + 1 }}</span>
          ...
          <template v-else>
            <div class="grid sm:grid-cols-2 gap-3">
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.phone') }}</span>
                <input v-model="form.phone" class="input mt-1" />
              </label>
              <label class="block text-sm">
                <span class="fk-label">{{ $t('schoolLandingEditor.email') }}</span>
                <input v-model="form.email" type="email" class="input mt-1" />
              </label>
            </div>
            <label class="block text-sm">
              <span class="fk-label">{{ $t('schoolLandingEditor.addressEn') }}</span>
              <textarea v-model="form.address_en" rows="2" class="input mt-1" />
            </label>
            <label class="block text-sm">
              <span class="fk-label">{{ $t('schoolLandingEditor.addressAr') }}</span>
              <textarea v-model="form.address_ar" rows="2" class="input mt-1" dir="rtl" />
            </label>
          </template>

          <div class="flex flex-wrap justify-end gap-2 pt-4 border-t border-fikr-hairline">
            <button
              type="button"
